###
# Makefile to build the Prometheus time series plugin from source
##

.DEFAULT_GOAL := prom-tss-plugin
...
help:
	@echo ""
	@echo "Build the Prometheus time series plugin from source"
	@echo ""
	...
	@echo "  prom-tss-plugin:       Compile the plugin from source code"
...
.PHONY prom-tss-plugin:
prom-tss-plugin: deps-build
...
release: deps-build
	...
	@$(MAKE) prom-tss-plugin >>$(RELEASE_LOG) 2>&1
	@echo "$(OK)"
	@echo -n "🎁 Git commit new release       "
	@git commit --signoff -am "release: Prometheus time series plugin version $(RELEASE_VERSION)" >>$(RELEASE_LOG) 2>&1
	@echo "$(OK)"
	@echo -n "🦄 Set Git version tag:         "
	@git tag -a "v$(RELEASE_VERSION)" -m "Release Prometheus time series plugin version $(RELEASE_VERSION)" >>$(RELEASE_LOG) 2>&1
	@echo "$(OK)"
	...
	@git commit --signoff -am "release: Prometheus time series plugin version $(SNAPSHOT_VERSION)" >>$(RELEASE_LOG) 2>&1
	@echo "$(OK)"
	@echo ""
